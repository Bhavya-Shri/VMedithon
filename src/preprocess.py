"""EEGMAT preprocess: physics match (P3, 10 ch, 1-40 Hz, 128 Hz, 2 s epochs).

Filter the whole recording, then window -- never filter per-epoch (Guide §20).
STEW preprocess is Phase 7; not in this file yet.
"""
...
from src.channels import canonicalize
...
def process_eegmat_file(path, label: int | None = None) -> dict | None:
    ...
    sfreq = float(raw.info["sfreq"])
    # MNE is Volts. Audit showed +/-2e-4 V. Missing *1e6 would break PTP (200 uV).
    data_uv = raw.get_data() * 1e6

    X = make_epochs(data_uv, sfreq)
    n_before = int(X.shape[0])
    thresh = PTP_REJECT_UV
    X, keep = reject_ptp(X, thresh)
    drop_frac = 1.0 - (X.shape[0] / n_before) if n_before else 0.0
    if n_before and drop_frac > PTP_DROP_FRAC:
        # Guide: if we bin >40% of a recording, loosen 200 -> 300 uV and retry.
        X_retry = make_epochs(data_uv, sfreq)
        X, keep = reject_ptp(X_retry, PTP_LOOSEN_UV)
